...
from settings.config import ATTEMPTS, WAIT_TIME
    ...
    while attempts <= ATTEMPTS:
        try:
            ...
            if not process_redact_note(driver, abs_data):
                attempts += 1
                if not click_cancel_note(driver):
                    need_reload = True
                continue
            print("Запись успешно отредактирована")

            if not click_save_note(driver):
                attempts += 1
                if not click_cancel_note(driver):
                    need_reload = True
                continue
...
def pipeline_run_task(driver, abs_data):
    ...
    while attempts <= ATTEMPTS:
        try:
            ...
            #    change_task_name(driver, abs_data['name'].append('+'))
    ...
    while attempts <= ATTEMPTS:
        try:
            ...
            if not pipeline_run_task(driver, abs_data):
                attempts += 1
                continue
            print(f"Задача '{abs_data['name']}' успешно обработана и запущена")
            return True
